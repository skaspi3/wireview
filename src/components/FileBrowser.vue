<template>
  <n-drawer v-model:show="isOpen" :width="520" placement="left" :trap-focus="true" @after-leave="$emit('close')">
    <n-drawer-content title="Open Capture File" closable>
      ...
      <div v-else class="file-list">
        ...
        <n-empty v-if="files.length === 0" description="No capture files found" style="padding: 40px 0;">
          <template #extra>
            <span style="color: #6b7280; font-size: 12px;">.pcap, .pcapng, .pcap.zst, .pcapng.zst</span>
          </template>
        </n-empty>
      </div>
